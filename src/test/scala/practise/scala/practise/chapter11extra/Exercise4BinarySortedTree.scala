  ...
  object Exercise {
    ...
    abstract class Tree(value: Int) {
      def contains(v: Int): Boolean = this match {
        case x: Leaf => this.value == v
        case Branch(value, _, _) if value == v => true
        case Branch (_, left, _) if left.contains(v) => true
        case Branch(_, _, right) if right.contains(v) => true
        case _ => false
      }
    }
    ...
    def createTree(values: List[Int]): Tree = values match {
      case x if x.size == 1 => Leaf(x(0))
      case x => Branch(x.find(y => y != x.min && y != x.max).get, Leaf(x.min), Leaf(x.max))
    }
  }
