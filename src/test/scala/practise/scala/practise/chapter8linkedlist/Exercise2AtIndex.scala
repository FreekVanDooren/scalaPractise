...
class Exercise2AtIndex extends FunSuite {
  object Exercise {
    ...
    abstract class AbstractLinkedList {
      ...
      def atIndex(index: Int): String = throw new IndexOutOfBoundsException
    ...
    case class LinkedList(value: String, tail: AbstractLinkedList) extends AbstractLinkedList {
      override //returns the value at 'index', may throw an IndexOutOfBoundsException
      def atIndex(index: Int): String = {
        index match {
          case 0 => value
          case _ => tail.atIndex(index - 1)
        }
      }
    }
  }
