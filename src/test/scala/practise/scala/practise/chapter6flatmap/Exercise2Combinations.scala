...
class Exercise2Combinations extends FunSuite {
  object Exercise {
    ...
    def combinationsForOneLetter(letter: String, numbers: List[Int]): List[String] = for {
      number <- numbers
    } yield letter + number
    ...
    letters flatMap {
      (letter) => combinationsForOneLetter(letter, numbers)
    }
    ...
    def combinationsFor(letters: List[String], numbers: List[Int]): List[String] = for {
      letter <- letters
      combinedLetters <- combinationsForOneLetter(letter, numbers)
    } yield combinedLetters
  }
  import Exercise._
